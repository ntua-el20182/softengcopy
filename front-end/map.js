...
fetch(localStorage.getItem('baseURL') + '/toll', {
    ...
    data.forEach(row => {
        ...
        if (!uniqueStations.has(row.name) || isSide) {
            ...
            // Ενημέρωση του popup με τα δεδομένα
            marker.bindPopup(`
                <div class="popup-content">
                    ...
                    <h5>Τιμές:</h5>
                    ${priceTable}
                    <a href="toll_statistics.html?toll_pk=${row.toll_pk}&name=${(row.name)}&dest=${(row.dest)}" class="popup-button" target="_blank" 
...
legend.onAdd = function (map) {
    ...
    div.innerHTML = `
        ...
        <div><span style="background: coral;"></span> Γέφυρα Ρίου - Αντιρρίου</div>
        <div><span style="background: green;"></span> Εγνατία Οδός</div>
